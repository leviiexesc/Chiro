    ...
    dispatch_once(&onceToken, ^{
        ...
        for (size_t i = 0; i < sizeof(candidates) / sizeof(candidates[0]); i++) {
            if (dlopen(candidates[i], RTLD_LAZY | RTLD_GLOBAL)) {
                NSLog(@"[Chiro] ls: loaded %s", candidates[i]);
                return;
            }
        }
        NSLog(@"[Chiro] ls: CoreServices/MobileCoreServices dlopen failed");
    });
}
...
static NSDictionary *appsFromWorkspace(void) {
    ...
    for (NSString *selectorName in @[@"allApplications", @"allInstalledApplications"]) {
        ...
        NSLog(@"[Chiro] ls: %@ returned %lu", selectorName,
              (unsigned long)([candidate isKindOfClass:[NSArray class]] ? [candidate count] : 0));
    }
    if (!apps || apps.count == 0) {
        NSLog(@"[Chiro] ls: workspace enumeration empty");
        return result;
    }
    NSLog(@"[Chiro] ls: %@ returned %lu proxies", usedSelector, (unsigned long)apps.count);
    ...
    NSLog(@"[Chiro] ls: extracted %lu apps (%lu with container)",
...
NSDictionary<NSString *, NSDictionary *> *installedAppInfo(void) {
    ...
    NSLog(@"[Chiro] ls: workspace empty; MobileInstallation=%lu",
