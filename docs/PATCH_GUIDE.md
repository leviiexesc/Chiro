...
Chiro patch packages target an app by its stable bundle identifier. They never store the app-container UUID, because that UUID changes across devices and reinstalls.
...
4. Tap **Done**. Chiro creates the project and its editable workspace.
5. Open **Files → Chiro Workspace → Patches → project name → bundle ID**.
...
3. Chiro captures the stable bundle identifier and relative path automatically and opens a patch draft.
...
- Writes are journaled and verified. If applying fails partway through, Chiro attempts to roll the transaction back.
...
- On a new device or installation, a protected package asks for its password once. Chiro stores the unlocked content key in Keychain; the exported package remains encrypted and tied to its original password.
